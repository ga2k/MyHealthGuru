...
namespace wx {
export class NamePage : public WizardPage {
   TextCtrl *m_GivenName {};
   TextCtrl *m_FamilyName {};

   /*
    * Args required by NamePage
    *
    * arg              type                    use
    * ---------------- ----------------------- --------------------------------------
 ...
 public:
   ~NamePage() override = default;
   explicit NamePage (wxWizard *parent, const std::string &pageName = "", const std::string &introduction = "",
                      const anymap &parameters = {})
      : WizardPage (parent, pageName, introduction, parameters) {
      TraceCall;

      bInitialRun = hs::param (parameters, "bInitialRun", true);
      layoutPath = hs::param (parameters, "layoutPath", "NamePage.yaml");
...
#define DFLT_LABEL_OPTS wxDefaultPosition, wxDefaultSize
      ...
      auto layoutFile {
         hs::resourceName (UIType::FormLayout, "NamePage.yaml", false, nullptr)}; // toml")}; // .json")};
      if ( !layoutFile.empty() ) {
